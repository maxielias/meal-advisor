...
def get_daily_meal_plan(df, filter_col, optimize_col, filters, tot_cal, n):
    ...
    for i, f in enumerate(filters):
        for _ in range(n[i]):
            ...
            df_to_append = df_filtered[df_filtered["diff"]==df_filtered["diff"].min()].reset_index(drop=True)
            if len(df_to_append) > 1:
                randinteger = randrange(0, len(df_to_append) + 1, 1)
                df_to_append = df_to_append.iloc[randinteger]
            else:
                pass
            df_meal_plan = pd.concat([df_meal_plan, df_to_append])
